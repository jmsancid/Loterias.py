...
def sqlConnection(db: [str, None] = None):
    """
...
def sqlInsert(con, tabla, campos, valores):
    cur = con.cursor()
    vals = '?' + ', ?' * (len(campos) - 1)
...
def sqlFind(con, tabla, campo, valor):
...
def sqlGetColumnName(con, tabla):
    cur = con.cursor()
...
def sqlCheckIfExists(con, tabla, valores, campos=None):
    """
...
def getLastCombinations(con, table, day=None):
    '''
    Devuelve la última combinacion almacenada en la tabla "table", de la base de datos "con", del sorteo del dia "day"
...
# NO UTILIZADA 20250109
def sqlGetColumn(con, tabla, campos, dias=(x for x in range(7)), ):
    """
